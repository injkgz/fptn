...
| `--max-ping` | `5000` (`0` — выключить) | Предел задержки в миллисекундах. Сервер сверх предела не выбирается, а действующий заменяется, если держится над пределом |
...
сервером. Замер, упавший по таймауту, считается превышением, так что переставший
отвечать сервер покидают за три минуты.

Сторож не создаётся в трёх случаях: `--max-ping` равен `0`, сервер закреплён
через `--preferred-server`, либо в пуле всего один сервер. Последнее важно на
роутере: переезжать некуда, а туннель при этом гасился бы каждые три минуты без
всякого толку — медленный сервер лучше, чем никакого.

Измеряется не ICMP-эхо. Проба открывает TLS-соединение к серверу — с той же
обфускацией и тем же SNI, что и сам туннель, — и качает тестовый файл на 100 КБ
(`/api/v1/test/file.bin`) с таймаутом в пять секунд. Поэтому здоровый дальний
сервер даёт сотни миллисекунд, а загруженный — секунду-другую, и дефолт стоит на
пяти секундах: это граница, за которой туннель уже не медленный, а нерабочий.
Цена — 100 КБ в минуту на клиента, и идут они мимо туннеля.

Провинившийся сервер не запоминается дольше текущего запуска: пул собирается
заново при каждом старте, и отброшенный за медлительность сервер меряют снова.
Он остаётся за бортом ровно пока остаётся медленным.
...
| `--probe-interval` | `180` | Перемерять весь пул раз в N секунд. `0` — не перемерять |
| `--switch-tolerance` | `500` | Насколько другой сервер должен быть быстрее, в миллисекундах, чтобы туннель ушёл на него сам. `0` оставляет только случай «перестал отвечать» |
...
При `--probe-interval 0` в окне остаётся только то, что измерилось при выборе
...
Поэтому обход включён по умолчанию.

Обход меряет все серверы, не больше восьми одновременно, — один мёртвый узел не
задерживает остальные. Проба — то же TLS-соединение, что открывает туннель, с
той же обфускацией и тем же SNI, а за ним запрос DNS-записи на несколько
десятков байт. Раньше качался тестовый файл на 100 КБ, и пул из тридцати пяти
серверов превращался в 3.5 МБ за проход ради числа, которое даёт уже сам
хендшейк.

### Когда клиент переезжает сам

После каждого обхода клиент сравнивает текущий сервер с лучшим из ответивших и
переезжает, когда это того стоит. Схема — urltest из sing-box, но пороги
выставлены под туннель, а не под outbound: sing-box переключается на разнице в
50 мс, потому что ему это ничего не стоит, а здесь это секунда тишины и новый
логин к серверу, который считает сессии на пользователя.

- **текущий сервер перестал отвечать или отвечает сверх `--max-ping`** —
  переезжаем сразу, ничего не дожидаясь;
- **иначе** — только если другой быстрее больше чем на `--switch-tolerance`, и
  только если прошлый переезд был больше десяти минут назад. Без этого порога
  пул из двух серверов с разницей в пару сотен миллисекунд менялся бы на каждом
  обходе.

Сравнивается среднее по окну измерений, а не последний замер: одиночная проба
врёт достаточно часто — сеть моргнула, узел был занят, — чтобы решение по ней
двигало туннель впустую.

Сам клиент не переезжает в четырёх случаях: сервер закреплён через
`--preferred-server`, в пуле один сервер, маршрутами владеет сам клиент (см.
ниже) или кто-то выбрал сервер через `PUT /proxies/<имя>`. Последнее сделано
намеренно: явный выбор старше любого измерения, и отмена его на следующем
обходе выглядела бы как борьба клиента с панелью. Действует до перезапуска
процесса.
